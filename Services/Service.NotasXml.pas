...
var
  ...
  LJsonES : TJSONObject; // "E" : { }  "S": { } - V�o ser agrupados dessa forma
...
//}
  ...
  try
    for I := 0 to LNotas.Count - 1 do begin
      ...
      // Pegando a posi��o do modelo na String do XML
  ...
  try
    ...
    try
      ...
      if LQueryXml.IsEmpty then begin
          TLogger.Info('N�o foram achados arquivos XML.');
          Result.AddPair('status', 'vazio');
          Exit;
      end;
      ...
      while not LQueryXml.Eof do begin
       ...
       LXmlStream := TStringStream.Create(LXmlDescompactado, TEncoding.UTF8);
         ...
         TLogger.Info('XML ignorado por corrup��o ou vazio. Chave: ' + LChaveStr);
       end;
    ...
    LEmailService.Enviar(
      ...
      'Ol�, segue em anexo os XMLs organizados',
      [LAnexoZip]
    );
